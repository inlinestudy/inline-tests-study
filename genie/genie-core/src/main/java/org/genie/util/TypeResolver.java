    ...
    public static void setup() {
        ...
        try {
            ...
            if (TypeResolverUtil.depClassPaths != null) {
                ...
                for (String path : TypeResolverUtil.depClassPaths.split(File.pathSeparator)) {
            ...
            if (TypeResolverUtil.appSrcPath != null) {
                // add all source paths to the solver
                for (String path : TypeResolverUtil.appSrcPath.split(File.pathSeparator)) {
                    if (Paths.get(path).toFile().isDirectory()) {
                        sTypeSolver.add(new JavaParserTypeSolver(path));
                    } else {
                        // assume it is a file
                        if (TypeResolverUtil.appSrcPath.contains("src/main/java")) {
                            sTypeSolver
                                    .add(new JavaParserTypeSolver(
                                            TypeResolverUtil.appSrcPath.split("src")[0] + "src/main/java"));
                        } else {
                            sTypeSolver
                                    .add(new JavaParserTypeSolver(
                                            Paths.get(TypeResolverUtil.appSrcPath).getParent().toString()));
                        }
                    }
                }
            }
